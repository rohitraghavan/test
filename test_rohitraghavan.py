...
def f03(*words):
    truth = " ".join(words)  # This is fixed
...
def f06(string1, string2):
    ...
    message1 = "{} is longer than {} by {} chars"
    message2 = "{} has only {:.2f} the number of chars of {}"
    if difference < 0:
        print(message1.format(string2, string1, -(difference)))
        percent = (len(string1) / len(string2)) * 100
        print(message2.format(string1, percent, string2))
        print()
    elif difference > 0:
        print(message1.format(string1, string2, difference))
        percent = (len(string1) / len(string2)) * 100
        print(message2.format(string1, percent, string2))
...
###############################################################################
def f09():
    return sum([counter for counter in range(0, 500) if counter %
                3 == 0 or counter % 5 == 0])
...
def f10(counter):
    no_sum = 0
    if counter < 499:
        no_sum = f10(counter + 1)
    if counter % 3 == 0 or counter % 5 == 0:
        return no_sum + counter
    else:
        return no_sum
...
def f11(args):
    if isinstance(args, str):
        print("{}".format(float(args)))
    if isinstance(args, float):
        print("{}".format(int(args)))
...
def f12():
	...
	faulty_input = ""
	while(True):
		raw_input_text = input("Enter a value ('done' to exit): ")
		if raw_input_text == 'done' or raw_input_text == '"done"':
			break
		try:
			float_input = float(raw_input_text)
		except Exception as e:
			faulty_input = faulty_input + raw_input_text + "\n"
		else:
			converted_input.append(float_input)
	print(converted_input)
	with open("log_file.txt","w") as log_file:
		log_file.write(faulty_input)
